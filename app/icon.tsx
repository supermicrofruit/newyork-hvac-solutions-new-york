...
export default function Icon() {
  // 1. If pipeline already generated a favicon, serve it directly
  const faviconPath = (businessData as any).theme?.favicon
  if (faviconPath) {
    const filePath = join(process.cwd(), 'public', faviconPath)
  ...
  const defaultFaviconPath = join(process.cwd(), 'public/images/favicon.png')
  if (existsSync(defaultFaviconPath)) {
    const buffer = readFileSync(defaultFaviconPath)
    return new Response(buffer, {
      headers: { 'Content-Type': 'image/png' },
    })
  }

  // 2. No pipeline favicon — generate one with first letter on accent bg
